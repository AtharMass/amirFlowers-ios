...
class Arrangements: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource {
    ...
    func loadDB() {
        dbRef.queryOrdered(byChild: "category").queryEqual(toValue: "סידורים").observeSingleEvent(of: DataEventType.value) { (snapshot) in
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        self.audioPlayer?.play()
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        if let v = storyboard.instantiateViewController(withIdentifier: "ArrangementsSDetail") as? ArrangementsSDetail {
